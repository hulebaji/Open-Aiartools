...
export function generateVerificationEmailHtml(verificationUrl: string, locale: string = 'en') {
  ...
  return `
    ...
    <body>
      ...
      <div class="content">
        <p>${isZh ? '您好！' : 'Hello!'}</p>
        <p>${isZh ? '感谢您注册须弥AI！请点击下面的按钮验证您的邮箱地址：' : 'Thank you for signing up for Showme AI! Please click the button below to verify your email address:'}</p>
        ...
        <p>
          ${isZh ? '如果您没有注册须弥AI账户，请忽略此邮件。' : 'If you did not sign up for Showme AI, please ignore this email.'}
        </p>
      </div>
      <div class="footer">
        <p>&copy; 2025 ${isZh ? '须弥AI' : 'Showme AI'}. ${isZh ? '保留所有权利。' : 'All rights reserved.'}</p>
      </div>
    </body>
    </html>
  `;
...
export function generatePasswordResetEmailHtml(resetUrl: string, locale: string = 'zh'): string {
  ...
  return `
    ...
    <head>
      ...
      <title>${isZh ? '须弥AI - 密码重置' : 'Showme AI - Password Reset'}</title>
      <style>
        body {
    ...
    <body>
      <div class="container">
        ...
        <div class="footer">
          <p>${isZh ? '此邮件由须弥AI自动发送，请勿回复。' : 'This email was sent automatically by Showme AI. Please do not reply.'}</p>
          <p>&copy; 2025 ${isZh ? '须弥AI' : 'Showme AI'}. ${isZh ? '保留所有权利。' : 'All rights reserved.'}</p>
        </div>
      </div>
    </body>
